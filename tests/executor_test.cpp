...
TEST_F(GEMM_Executor_Test, Correctness_Single) {
  ...
  for (auto &opts : GEMM_Options::enumerate()) {
    TestMatrix<float> A(m,k,m);
    TestMatrix<float> B(k,n,k);
    TestMatrix<float> C(m,n,m);
    GEMM_Inputs<float> inputs(handle, CUBLAS_OP_N, CUBLAS_OP_N, A, B, C, alpha, 0.0);
    ...
    C.download();
    EXPECT_TRUE(!C.is_zero());
    test_gemm<float>(A, B, C, -alpha, 1.0, false, false);
    EXPECT_TRUE(C.is_zero());
  }
  for (auto &opts : GEMM_Options::enumerate()) {
    TestMatrix<float> A(m,k,m);
    TestMatrix<float> B(n,k,n);
    TestMatrix<float> C(m,n,m);
    GEMM_Inputs<float> inputs(handle, CUBLAS_OP_N, CUBLAS_OP_T, A, B, C, alpha, 0.0);
    ...
    C.download();
    EXPECT_TRUE(!C.is_zero());
    test_gemm<float>(A, B, C, -alpha, 1.0, false, true);
    EXPECT_TRUE(C.is_zero());
  }
  for (auto &opts : GEMM_Options::enumerate()) {
    TestMatrix<float> A(k,m,k);
    TestMatrix<float> B(k,n,k);
    TestMatrix<float> C(m,n,m);
    GEMM_Inputs<float> inputs(handle, CUBLAS_OP_T, CUBLAS_OP_N, A, B, C, alpha, 0.0);
    ...
    C.download();
    EXPECT_TRUE(!C.is_zero());
    test_gemm<float>(A, B, C, -alpha, 1.0, true, false);
    EXPECT_TRUE(C.is_zero());
  }
  for (auto &opts : GEMM_Options::enumerate()) {
    TestMatrix<float> A(k,m,k);
    TestMatrix<float> B(n,k,n);
    TestMatrix<float> C(m,n,m);
    GEMM_Inputs<float> inputs(handle, CUBLAS_OP_T, CUBLAS_OP_T, A, B, C, alpha, 0.0);
    ...
    C.download();
    EXPECT_TRUE(!C.is_zero());
    test_gemm<float>(A, B, C, -alpha, 1.0, true, true);
    EXPECT_TRUE(C.is_zero());
  }
  SUCCEED();
}
